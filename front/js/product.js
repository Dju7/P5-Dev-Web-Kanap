...
if (!urlId) {
  ...
  function validateSelection() {
    ...
    if (
      isNaN(selectQuantity) ||
      selectQuantity < 1 ||
      !Number.isInteger(parseInt(selectQuantity)) ||
      selectQuantity < 1 ||
      selectQuantity > 100
    ) {
      alert(
        "Vous devez entrer une quantité numérique entre 1 et 100 pour ajouter un produit au panier!"
      );
      return;
    }
    ...
    if (selectColor === "") {
      alert(
        "Vous devez sélectionner une couleur pour ajouter un produit au panier!"
      );
      return;
    }
    return true;
    ...
    btnBasket.addEventListener("click", () => {
      if (validateSelection()) {
        ...
        const productExist = cart.find(
          (item) => item.id === addProduct.id && item.color === addProduct.color
        );
        if (productExist) {
